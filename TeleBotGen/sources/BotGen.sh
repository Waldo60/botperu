...
[[ ! -e "/bin/ShellBot.sh" ]] && wget -O /bin/ShellBot.sh https://raw.githubusercontent.com/Waldo60/botperu/main/ShellBot.sh &> /dev/null
...
reply () {
	...
	[[ "${callback_query_data}" = /del || "${message_text}" = /del ]] && listID_src
      #[[ "${callback_query_data}" = /killid || "${message_text}" = /killid ]] && listID_src
}
...
invalido_fun () {
	[[ ! -z ${callback_query_message_chat_id[$id]} ]] && var=${callback_query_message_chat_id[$id]} || var=${message_chat_id[$id]}
local bot_retorno="  🤖Bienvenido  al  BOT_GEN VPS-OO🤖 \n"
	 bot_retorno+="$LINE\n"
...
ShellBot.InlineKeyboardButton --button 'botao_conf' --line 1 --text 'Add ID' --callback_data '/add'
ShellBot.InlineKeyboardButton --button 'botao_conf' --line 1 --text 'Del ID' --callback_data '/del'
...
ShellBot.InlineKeyboardButton --button 'botao_donar' --line 2 --text 'ACCEDER TELEGRAM' --callback_data '1' --url 'https://t.me/Gen_bot'
